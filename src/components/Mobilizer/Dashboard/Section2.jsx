    ...
    <section className="grid grid-cols-1 mt-6 p-6 rounded-2xl
      ...
      <div className="bg-[#111827] border border-yellow-400 rounded-2xl p-5">
        <div className="relative flex justify-between items-center mb-4">
          ...
          <button
            ...
            className="font-semibold text-white/90 hover:text-yellow-400"
          >
            {months[month]} {year}
          </button>
            ...
            >
              <div className="mb-3">
                ...
                <select
                  ...
                  className="w-full mt-1 bg-[#111827] border border-yellow-400 rounded-md px-2 py-1 text-white/90"
                >
                  {months.map((m, i) => (
                ...
                className={`rounded-full py-2 text-sm font-medium transition
                  ${
                    active
                      ...
                      : "bg-white/5 text-white/60 hover:bg-yellow-400"
                  }`}
              >
                {day}
              </button>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-6 bg-[#111827] border border-yellow-400 rounded-2xl p-6">
          <div>
            <p className="text-sm text-white/60">Mobilized (Month)</p>
            <p className="text-3xl font-bold text-slate-100">
              {monthStats.mobilized}
            </p>
          </div>
          <div>
            <p className="text-sm text-white/60">Enrolled (Month)</p>
            <p className="text-3xl font-bold text-slate-100">
              {monthStats.enrolled}
            </p>
          </div>
      ...
      <div className="bg-[#111827] border border-yellow-400 rounded-2xl p-6 text-center">
        ...
        <div className="text-sm text-left space-y-2 text-white/80">
          <p><strong className="text-white/60">Age:</strong> {mobilizer.age}</p>
          <p><strong className="text-white/60">Gender:</strong> {mobilizer.gender}</p>
          <p><strong className="text-white/60">Email:</strong> {mobilizer.email}</p>
          <p><strong className="text-white/60">Phone:</strong> {mobilizer.phone}</p>
          <p><strong className="text-white/60">Designation:</strong> {mobilizer.designation}</p>
          <p><strong className="text-white/60">Location:</strong> {mobilizer.location}</p>
        </div>
      </div>
    </section>
  );
