...
const STEPS = [
  ...
  { label: "Assign Projects", icon: Briefcase },
...
export default function SuperAdminProjectAssignment() {
  ...
  const canNext = () => {
    if (step === 0) return selectedAdmin !== null;
    if (step === 1) return selectedProjects.length > 0;
    if (step === 2) return selectedCenters.length > 0;
    return true;
  };

  const handleSubmit = () => navigate("/super-admin/project-assignment");
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-100 flex items-center gap-3 tracking-tighter">
            <FolderKanban size={28} className="text-red-500" /> Project & Center Assignment
          </h1>
          <p className="text-sm text-white/60 mt-1 uppercase tracking-widest font-bold">Assign projects and centers to project leads</p>
        </div>
      ...
      <div className="bg-[#111827]/80 border border-slate-700/50 rounded-2xl p-8 backdrop-blur-sm">
              ...
              {ADMINS.map((admin) => (
                <button key={admin.id} onClick={() => setSelectedAdmin(admin)}
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-white/90">{admin.name}</p>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              {PROJECTS.map(p => (
                <button key={p} onClick={() => toggle(p, selectedProjects, setSelectedProjects)}
                  ...
                  {selectedProjects.includes(p) ? <CheckCircle2 size={18} /> : <Briefcase size={18} className="text-slate-600" />}
                  {p}
                </button>
              ))}
            </div>
        ...
        {step === 3 && (
          <div className="space-y-6">
            ...
            <p className="text-xs text-slate-500">Verify the assignment details before confirming.</p>
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="p-4 bg-transparent/30 rounded-xl border border-slate-700/50">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Admin</p>
                  <p className="text-sm font-bold text-white/90">{selectedAdmin?.name}</p>
                  <p className="text-[10px] text-slate-500 mt-0.5">{selectedAdmin?.id} • {selectedAdmin?.center}</p>
                </div>
                <div className="p-4 bg-transparent/30 rounded-xl border border-slate-700/50">
